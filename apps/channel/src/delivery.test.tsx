...
import { MentionGroupCard } from "./components";
...
class ResearchAgent extends AbstractAgent {
  ...
  run(input: RunAgentInput): Observable<BaseEvent> {
    const calls = [
      { name: "search_web", args: { query: "cobro duplicado", results: 1 } },
      {
        ...
        args: {
          ...
          fuentes_distintas: 2,
          es_incidente: true,
          fuentes: [{ titulo: "Hilo en foro", url: "https://example.com/hilo" }],
        },
      },
    ];
  ...
  try {
    await gateway.deliver(
      preparedDelivery("research", "slack", {
        kind: "text",
        text: "Analiza la reputacion y muestra una tarjeta",
      }),
    );
    return {
      gateway,
...
it(
  ...
  async () => {
    ...
    assert.match(JSON.stringify(cards[1]), /Cobro duplicado tras cancelar/);
